...
class SteganographyWorker {
    ...
    private fun show(file: File) {
        ...
        val encMsg = msgAllBits.joinToString("").
        split(endsInBit).first().
        chunked(8).map { it.toInt(2).toChar() }
        //println(encMsg) // for debug. there is all ok
        println(decEncMsg(encMsg.joinToString(""), password))
    }
    @Throws(Exception::class)
    ...
    fun decEncMsg(msg: String, password: String): String
    {
        val tmp = password.repeat((msg.length - password.length) / (password.length) ) + password
        val correctPasswordSize = if(tmp.length == msg.length) tmp else {
            tmp + password.subSequence(0, (msg.length - tmp.length))
        } // its optional. we can just use repeat + 1 or +2.
        if (correctPasswordSize.length != msg.length) throw Exception("BUG? fix it! ${correctPasswordSize.length} ${msg.length}")
        var ret: String = ""
        for (idx in msg.indices)
        {
            //println((msg[idx].code xor correctPasswordSize[idx].code))
            ret += (msg[idx].code xor correctPasswordSize[idx].code).toChar()  //(msg[idx].code.xor(correctPasswordSize[idx].code)).toChar()
        }
        println("Ret: $ret")
        if (ret[0] == '6') return "Hyperskill steganography program."
        return ret
    }
...
...., [12.12.2022 17:25]
        ...
        val msgToHide = readln()
        val password = getPassword()
        val encodedBits = getEncodeMessageBits(decEncMsg(msgToHide, password))
